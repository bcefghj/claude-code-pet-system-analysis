...
BUDDY 的动画由 `CompanionSprite.tsx` 驱动，使用 React/Ink 在终端中实时渲染：

#### 动画时钟
- **TICK_MS = 500**：每 500 毫秒一个 tick
- **IDLE_SEQUENCE = [0,0,0,0,1,0,0,0,-1,0,0,2,0,0,0]**：15 步循环
  - `0` = Frame 0（休息姿态）
  - `1` = Frame 1（微动/变化）
  - `2` = Frame 2（特殊动作，如龙喷烟、蘑菇散孢子）
  - `-1` = 眨眼（眼睛字符替换为 `-`）

#### 精灵渲染 (`renderSprite`)
- 每种宠物 5 行 x 12 字符宽（`{E}` 替换为 1 字符后）
- 3 帧动画数据存储在 `BODIES` 记录中
- 第 0 行为帽子槽位：常态为空白，帽子渲染时替换
...
#### 抚摸反馈
- 点击宠物触发 `PET_BURST_MS = 2500ms` 心形动画
- 5 帧心形飘浮（`♥ ♥ ♥ → · · ·`）
- 触发时动画变为快速帧循环（`tick % frameCount`）

#### 对话气泡
- **BUBBLE_SHOW = 20 ticks**（~10 秒）
- **FADE_WINDOW = 6 ticks**（最后 ~3 秒渐隐）
- 宠物名字被提及时自动触发

#### 窄屏模式
- 终端宽度 < 100 列时切换为 `renderFace()` 单行模式
- 示例：`(°>) goose` 或 `=·ω·= "Looking good!"`

#### 抽卡逻辑 (`companion.ts`)
```
mulberry32(seed)  →  PRNG
hashString(userId + "friend-2026-401")  →  seed
rollRarity(rng)   →  加权随机（60/25/10/4/1）
rollStats(rng, rarity)  →  peak属性+50, dump属性-10
rng() < 0.01  →  Shiny 闪光判定
common 稀有度 = 无帽子
```

#### 通知系统 (`useBuddyNotification.tsx`)
- 2026年4月1日-7日窗口期，彩虹文字 `/buddy` 预热提示
- 随机对话内容自动触发

### 3.11 在线体验

本仓库包含一个忠实复刻官方源码逻辑的 **Web 宠物体验系统**：

**→ 打开 `buddy-pet-demo/index.html` 即可体验**

功能完整复刻：
- 18 种宠物 ASCII 精灵 + 3 帧动画
- mulberry32 PRNG 抽卡（与官方一致）
- 500ms/tick 动画循环 + IDLE_SEQUENCE
- 6 种眼睛 × 8 种帽子定制
- 5 种稀有度 + 1% Shiny 闪光
- 5 大属性条可视化
- 对话气泡 + 抚摸心形
...
### 源码级精确图（基于真实泄露代码）
1. `sprites_3_frames_all.png` - **18种宠物×3帧动画全展示**
2. `sprites_blink_states.png` - **眨眼状态对比**（6种宠物正常 vs 眨眼）
3. `sprites_hats_capybara.png` - **8种帽子展示**（Capybara × 所有帽子）
4. `sprites_eyes_cat.png` - **6种眼睛展示**（Cat × 所有眼睛 + renderFace 输出）
5. `sprites_rarity_shiny.png` - **稀有度+闪光对比**（5等级 + Shiny 变体）
6. `sprites_interaction_states.png` - **交互状态展示**（气泡/心形/窄屏/兴奋）

### 之前生成的总览图
7. `real_18_species_ascii.png` - 真实18种宠物ASCII图鉴
8. `real_buddy_customization.png` - 宠物定制系统展示
9. `buddy_18_species.png` - 18种宠物像素风图鉴
10. `buddy_stats_rarity.png` - 性格属性与稀有度系统
11. `buddy_system_overview.png` - Buddy 内置系统概览
12. `pet_evolution_system.png` - 进化系统全貌（社区版）
13. `pet_stats_and_moods.png` - 宠物属性和情绪界面
14. `pet_ecosystem_overview.png` - 宠物生态系统全览
15. `clawd_12_states.png` - Clawd on Desk 12种动画状态
